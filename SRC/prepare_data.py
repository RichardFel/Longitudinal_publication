...
def prepare_data(file, settings):
    # Load raw sensor data from file
    data_df = pd.read_csv(
        f"{settings['DATA_DIR']}/raw_data/{file}", header=None, skiprows=10,on_bad_lines = 'skip' )
    ...
    timestamps = pd.read_csv(
        f"{settings['DATA_DIR']}/raw_data/{file}", skiprows=8, on_bad_lines='skip')
